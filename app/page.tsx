...
function DashboardContent() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="border-4 border-black rounded-xl p-6 sm:p-8 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div>
          <h1 className="font-black text-2xl sm:text-4xl mb-2">{getGreeting()}, User!</h1>
          <p className="text-gray-600 font-bold text-sm sm:text-lg max-w-2xl">Log in now to save your progress securely and access your expenses on every device</p>
        </div>
        <Link href="/add" className="w-full sm:w-auto text-center border-3 border-black rounded-xl px-6 py-3 bg-black text-white font-black hover:bg-gray-800 transition-colors shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] whitespace-nowrap">
          + Add Expense
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        <Link href="/add" className="border-4 border-black rounded-xl p-4 sm:p-6 bg-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,0.3)] transition-all text-center font-bold text-sm sm:text-base">
          ➕ Add Expense
        </Link>
        <Link href="/expenses" className="border-4 border-black rounded-xl p-4 sm:p-6 bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] transition-all text-center font-bold text-sm sm:text-base">
          📊 See All Expenses
        </Link>
        <Link href="/analytics" className="border-4 border-black rounded-xl p-4 sm:p-6 bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] transition-all text-center font-bold text-sm sm:text-base sm:col-span-2 md:col-span-1">
          📈 Full Analytics
        </Link>
      </div>
    </div>
